...
Active work unit: 010-angular-write-actions
...
- 010 ações de escrita (favorite/follow/comment/delete + ArticleActions sem bug):
  build + 69 testes verdes. Aguardando revisão humana do PR. `src/` legado intocado.

## Telas/funcionalidades migradas vs. placeholders
- Migradas: `/` (home/feed), `/login`, `/register`, `/article/:slug` (leitura + ações de escrita).
...
- Mutações exigem auth (anônimo → /register); markdown sanitizado (sem bypass).
...
3. Próximas slices Fase 3 (ordem sugerida):
   - Editor (POST/PUT artigo) — Categoria C (cria/edita conteúdo autenticado).
   - Profile (GET perfil + abas "My Articles"/"Favorited") + Settings (update + **logout**).
   - Migrar a suite E2E da Fase 2 (hashbang → URLs limpas) e **atualizar o baseline**
     de `article-actions` (agora funciona autenticado).
...
- Não permitir mutação autenticada sem checagem de sessão (anônimo → /register).
